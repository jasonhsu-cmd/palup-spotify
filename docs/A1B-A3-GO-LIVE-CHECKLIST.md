...
| P3 | **Producer failure metric + alert** — a silently-failing producer = facts quietly going stale (today only `console.error`). | **MET (code)** — a facts write failure now raises a **stably-keyed** `ALERT product_facts_{upsert,audit}_failed tenant=… error=…` marker. **Deploy step (still OPEN):** configure a Cloud Logging **log-based metric + alert** on `product_facts_*_failed` so it pages, not just logs. Tested (the marker fires). | `PRODUCT_FACTS_POLL` canary |
