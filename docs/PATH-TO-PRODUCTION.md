...
| Live-quality measurement (real signal, not operator-attested) | BUILD | **wired, dark-safe** — measured-outcome ledger read into `POST /api/monitor` + the gate with a power floor (#372/#354/#378); `control-plane/src/champion-promoter.ts:216-228` | monitor/rollback **can** react to a measured signal, but fall back to the attested `qualityScore` until the outcome ledger has statistical power (order-attribution is live on the staging dev store but low-volume) |
