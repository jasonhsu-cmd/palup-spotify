...
**Operator note (final-review fix, whole-branch review 2026-08-23):** enabling the backfill write-plane (`CATALOG_BACKFILL_ENABLED=true`) **REQUIRES** wiring `catalogProductAdminSource` into `reconcileDeps` in the **same change** — else the delta clobber returns (a thin webhook/poll write silently nulls a rich Bulk-Ops backfill row, exactly the failure Task 6/7's review ruling closed). `server.ts`'s composition root now enforces this structurally: `catalogProductAdminSource` is built from `makeMultiTenantCatalogProductAdminSource` (`jobs/catalog-backfill.ts`) whenever `CATALOG_BACKFILL_ENABLED && ADMIN_TOKEN_CUSTODY_ENABLED && adminTokens` are all present, and `buildServer` **refuses to boot** if `reconcileDeps.catalogProduct` is wired while `catalogProductAdminSource` is not — so turning on `CATALOG_BACKFILL_ENABLED` alone, without also turning on `ADMIN_TOKEN_CUSTODY_ENABLED` (with a real admin-token store), fails loudly at startup instead of silently reintroducing the clobber. See `test/server-catalog-sync-wiring.test.ts`'s "FAILS FAST…"/"boots and wires the paired admin-shape source…" tests.
