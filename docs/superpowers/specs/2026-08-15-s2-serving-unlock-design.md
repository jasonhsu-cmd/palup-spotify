...
6. **`CART_LINE_ITEMS` now coexists with the `CATALOG_RETRIEVAL` shell render path — RESOLVED 2026-08-17 (was: a HARD co-enablement gate until S3/S4).** The shell path built `ctx` with `products:[]`, so `renderCartBlock` resolved zero cart items and the cart block silently dropped, with no audit flag. Fixed: on the retrieval path the brain resolves the cart's line-item ids via `GroundingPort.getProductsByIds` (a bounded by-id fetch over the existing Storefront `nodes(ids:)`, capped at `MAX_CART_LINE_ITEMS`=30 ids — never the full catalog), and feeds those into `renderCartBlock`. A fetch failure fail-closes to no cart block plus a `cart:byid_unavailable` audit flag. Both the audit-flag and the cart-via-corpus resolution this item named as S3/S4 work are now shipped, dark behind the off-by-default `CART_LINE_ITEMS`.
