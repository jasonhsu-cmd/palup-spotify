...
| C2 | **Subject-scoped auth — closed end-to-end for a verified shopper's OWN `/chat`, `/consent`, and `/forget` calls, as of PR #152.** Widget auth binds the *tenant*; the shopper subject is additionally bound to the verified `x-shopper-token` principal (`memorySubjectId`, `identity.ts`), gated on the explicit `verified` flag and the cross-shop tenant check, on all three surfaces — the shipped widget now sends that header on all three (previously `/consent`/`/forget` did not: a signed-in shopper's own consent/erasure calls looked anonymous to the server, so `/forget` returned `{ok:true}` while erasing nothing — PR #152 BLOCK-2). A guest-recorded consent choice (in particular an explicit opt-out) is also restrictively merged into the account record on sign-in (PR #152 BLOCK-1, `mergeAccountConsent`), so sign-in can no longer silently re-permit a write the shopper had opted out of. Guests remain principal-less by construction. | Closed for an authenticated shopper's own calls; the residuals are exactly C1's guest-bearer-capability case and B12's un-migrated guest-era facts, not separate risks introduced here. |
