...
| C6 | **A signed-in shopper's "forget everything" does NOT erase their guest-era facts, while the UI reports success.** With a verified token `/forget` targets `acct:<shopperId>` and ignores any supplied `anonId`, so facts written before sign-in survive and the widget still renders *"Done — I've cleared what I remembered."* PR #152 did not create the stranding (B12 did) but it **inverted** which side is affected: before, account data was un-erasable; now, guest-era data is. | INERT today (nothing is stored), but it also breaks **B4**: these namespaces are visited by no `sweepExpired` caller, so their facts outlive their TTL indefinitely. (An earlier draft of this row claimed erasing the supplied `anonId` too would reintroduce the C1 delete attack — that was WRONG: the guest path already lets an *unauthenticated* caller erase any `anonId` they hold, so doing it on a verified turn grants an attacker nothing they cannot get by omitting the token.) Fix: erase and sweep BOTH the account subject and the request's validated guest `anonId`, and/or wire B12's migration. |
| C7 | **A stale guest opt-out defeats that same shopper's later, authenticated opt-IN.** Guest records `"out"`, shopper signs in and explicitly grants consent against `acct:<id>` — the read-time merge still resolves `"out"`, so no writes occur, while the manage panel (local state) shows memory as on. | Fails in the privacy-safe direction, and only while the client keeps presenting the old `anonId`. No shopper-visible way to clear the stale guest record; resolved by the same B12 migration. |
