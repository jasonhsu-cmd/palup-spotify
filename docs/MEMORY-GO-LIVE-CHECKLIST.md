...
| B12 | **Guest → account memory continuity.** `mergeGuestIntoAccount` exists but has **no production caller**, and subject-scoped auth now makes the subject *switch* at sign-in (`anonId` → `acct:<id>`). So a shopper who used memory as a guest and then signs in starts a fresh subject: their guest facts are stranded under the old namespace and their guest consent record does not resolve (fail-closed `"unknown"` — safe, but memory silently stops working for someone who did consent). Harmless while inert (nothing is stored yet); **must be wired before go-live**, migrating the consent record alongside the facts. | **OPEN — not built** |
...
|---|---|---|
| C1 | **`anonId` is a bearer capability — for ANONYMOUS GUESTS ONLY (narrowed).** For a **signed-in** shopper this is now **closed**: the memory subject is `acct:<shopperId>`, derived from the server-verified principal, and a supplied `anonId` is ignored — so holding someone's `anonId` no longer lets you set their consent or `/forget` their memory. For a guest there is no principal to bind to, so possession of their `anonId` still is the capability. | 128-bit CSPRNG (not enumerable), per-tenant, rate-limited, kill-guarded. Blast radius is the victim's own preferences; no read access, no cross-tenant reach, orders/account untouched. Sign-off now covers guests only. |
| C2 | **Subject-scoped auth — DONE for verified shoppers.** Widget auth binds the *tenant*; the shopper subject is now additionally bound to the verified `x-shopper-token` principal (`memorySubjectId`, `identity.ts`), gated on the explicit `verified` flag and the cross-shop tenant check. Guests remain principal-less by construction. | Closed for authenticated shoppers; the residual is exactly C1's guest case, not a separate risk. |
