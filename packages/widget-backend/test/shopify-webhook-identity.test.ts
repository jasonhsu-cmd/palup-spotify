...
//
// FIX ROUND 1 — precision. Shopify's webhook body carries the id in two fields: the numeric `id` (a JSON
// number, already lossy for a large id — JSON.parse/the JS number type round anything beyond
// Number.MAX_SAFE_INTEGER before this function ever runs) and `admin_graphql_api_id`, a GID STRING
// "gid://shopify/Product/<id>" that is never subject to float64 rounding. This module reads the GID
// FIRST, as the precision-safe source, and falls back to the numeric `id` only when no GID is present.

describe("S3 §C — productIdOf: the changed product id from a products/* body", () => {
  it("reads the id from admin_graphql_api_id (the precision-safe source), even when a numeric id is also present", () => {
    expect(
      productIdOf({ id: 7, admin_graphql_api_id: "gid://shopify/Product/7" } as Record<string, unknown>),
    ).toBe("7");
  });

  it("reads a realistic 13-digit id delivered via admin_graphql_api_id", () => {
    expect(
  ...
  it("preserves an OVERSIZED (18-digit, beyond Number.MAX_SAFE_INTEGER) id EXACTLY via the GID string", () => {
    // This is the property fix round 1 exists for. If this read the numeric `id` field instead, the
    // JS number would already have rounded 788032119674292922 to 788032119674292900 by the time this
    // function runs — verified this session: `Number(788032119674292922) === 788032119674292900` — so
    // an implementation reading the numeric field CANNOT return the exact original digits here; only
    // reading the GID STRING can. A regression to a numeric-field read would fail this exact assertion.
    expect(
      productIdOf({ id: 1, admin_graphql_api_id: "gid://shopify/Product/788032119674292922" } as Record<string, unknown>),
    ).toBe("788032119674292922");
  });

  it("falls back to the numeric id when admin_graphql_api_id is absent", () => {
    expect(productIdOf({ id: 7880321196 } as Record<string, unknown>)).toBe("7880321196");
  });

  it("falls back to an all-digits string id when admin_graphql_api_id is absent", () => {
