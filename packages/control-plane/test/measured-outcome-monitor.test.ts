...
const mkEngine = () =>
  new EvolutionEngine({
    champion: {
      ...
      metrics: { policyId: DEFAULT_POLICY.id, safetyPass: true, floorPass: true, qualityScore: 0.75, counterMetrics: BASE_CM, measuredOutcome: { incrementalLift: 0.05, power: 0.95 } } as PolicyMetrics,
    },
...
describe("monitorServing — measured-outcome preference (Wave-2 D)", () => {
  ...
  it("(e) a trustworthy, comparable measured-lift REGRESSION rolls back serving even though qualityScore looks healthy", async () => {
    ...
    const r = await monitorServing(engine, store, T, {
      qualityScore: 0.99,
      safetyPass: true,
      measuredOutcome: { incrementalLift: 0.01, power: 0.95 },
    });
  ...
  it("a trustworthy, comparable measured lift that did NOT regress is healthy — confirms known-good, never touches serving", async () => {
    ...
    const r = await monitorServing(engine, store, T, {
      ...
      safetyPass: true,
      measuredOutcome: { incrementalLift: 0.08, power: 0.95 },
    });
  ...
  it("an UNDERPOWERED measured lift falls back to the caller-attested qualityScore ('else keep attested')", async () => {
    ...
    const r = await monitorServing(engine, store, T, {
      ...
      safetyPass: true,
      measuredOutcome: { incrementalLift: -50, power: 0.1 }, // present but far below the floor
    });
  ...
  it("a safety failure rolls back regardless of a positive measured lift", async () => {
    ...
    const r = await monitorServing(engine, store, T, {
      qualityScore: 0.99,
      safetyPass: false,
      measuredOutcome: { incrementalLift: 1000, power: 0.99 },
    });
