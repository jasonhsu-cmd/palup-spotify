...
describe.skipIf(!PGVECTOR_AVAILABLE)("PgVectorStore erasure + isolation", () => {
  it(
    "deleteById removes only given ids; deleteNamespace erases the tenant; namespaces isolated",
    async () => {
      // The 120_000 belongs on THIS `it` (3rd arg) — it boots+tears down a container inline, like its
      // pgvector-store.*.test.ts siblings. It was previously passed as a 2nd arg to `withPgvector`
      // (which takes only one param and silently ignores it), leaving `it` at vitest's 5s default —
      // flaky under S4's 10-container test:pgvector contention. Match the sibling pattern.
