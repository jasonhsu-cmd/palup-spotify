...
describe("retention — TTL day-counts (Inv 4/9: special is strictly shorter than ordinary)", () => {
  it("SPECIAL_TTL_DAYS < ORDINARY_TTL_DAYS", () => {
    expect(SPECIAL_TTL_DAYS).toBeLessThan(ORDINARY_TTL_DAYS);
  });
  ...
  it("ttlForClass mirrors the day-counts (in milliseconds)", () => {
    ...
    expect(ttlForClass("special")).toBeLessThan(ttlForClass("ordinary"));
  });
});
